...
class GoogleDriveService:
    ...
    async def process_folder(
        ...
        """Process a Google Drive folder recursively with token refresh capability."""
        try:
            logger.info(f"Processing folder {folder_id} for owner {owner_id}")
    ...
    async def process_file(
        ...
        """Process a single Google Drive file with token refresh capability."""
        if not file:
        ...
        try:
            ...
            async def download_with_refresh():
                ...
                try:
                    logger.info(f"Downloading with access token (first 10 chars): {access_token[:10]}...")
                    credentials = Credentials(token=access_token)
                    service = build('drive', 'v3', credentials=credentials)
                    ...
                    try:
                        ...
                        return file_content, None  # Returning file content and file_data
                        ...
                        if download_error.resp.status == 401 and refresh_token:  # Token expired
                            ...
                            logger.info("Token refreshed successfully")
                            return await download_with_refresh()
                        logger.error(f"Error downloading file {file['id']}: {download_error.resp.status} - {download_error._get_reason()}")
                        raise
            ...
            file_content, file_data = await download_with_refresh()
            if not file_content:
                logger.warning(f"No content retrieved for file {file['id']}")
                return

            # Process the document using the shared service
            file_metadata={
                    'filename': file['name'],
                    'file_id': file['id'],
                    'mime_type': file['mimeType'],
            ...
            logger.info(f"Processing document with metadata: {file_metadata}")
